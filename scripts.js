...
// guardar db
const GuardarDB = () => {
  localStorage.setItem('rutina', JSON.stringify(arrayLista));
  PintarDB();
}

const PintarDB  = () =>{
  listaTareasUI.innerHTML = '';
  arrayLista = JSON.parse(localStorage.getItem('rutina'));
  // console.log(arrayLista);
  if(arrayLista === null){
    ...
    arrayLista.forEach(element => {
      ...
      listaTareasUI.innerHTML +=    `<li>${element.actividad} </li><button  id="btn-editar">Editar</button><button id="btn-borrar">Eliminar</button>`
    });
  }
}
//EVENT LISTENERS 
